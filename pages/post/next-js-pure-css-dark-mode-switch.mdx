---
canonicalUrl: https://pgarciacamou.dev/post/next-js-pure-css-dark-mode-switch
author: Pablo Garcia
date: "2022-12-13"
edited: "2022-12-13"
...
<div editornotes="true">
  Should I start the names with an id so that they are sorted, for example:
  20230108-next-js-pure-css-dark-mode-switch.
</div>

# Next.js Pure CSS dark mode switch

Normally, you require JavaScript to make updates in the DOM so that we can apply certain clases, but we can use a vanilla HTML/CSS solution to also apply styles to the DOM depending on the user's selection. This idea is interesting when you think about static content, and you want to allow the user to access the content without any JavaScript needed on the client side.
...
```css title="theme.css" /background-color/13 /color/12 {2,16}
:root,
...
```css title="someComponent.module.css"
.someComponent {
  color: var(--theme-color);
}
```
